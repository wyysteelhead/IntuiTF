...
It contains utility functions for volume rendering, including:
...
def random_point_on_sphere(N: int, *, dtype=np.float32) -> Tuple[np.ndarray, np.ndarray]:
    """
    Generates N random points on the surface of a sphere.
    :param N: the number of points
    ...
    # longitudes (yaw) between 0 and 2π
    lon = 2 * np.pi * np.random.rand(N, dtype=dtype)
